...
bool MMMA8653_Init(void)
{
    ...
    if ( false == MMA8653_FreefallMotionConfigReg_Set ((MMA8653_MOTION_DETECTION | MMA8653_ZEFE | MMA8653_YEFE | MMA8653_XEFE)) ) return false; 
	  ...
	  if ( false == MMA8653_FreefallMotionThresholdReg_Set( MMA8653_MOTION_THRESHOLD_5G | MMA8653_DBCNTM_SET) ) return false;
	
	  /* Set the number of debounce sample counts for the event trigger */
	  if ( false == MMA8653_DebounceRegister_Set(MMA8653_DEBOUNCE_COUNT_5) ) return false;
    ...
    if ( false == MMA8653_ASLPCounter_val(MMA8653_30S_TIMEOUT) ) return false;  
    ...
    if ( false == MMA8653_AutoSleep_Enable() ) return false;  
    ...
    if ( false == MMA8653_InterruptConfigurationReg_config(MMA8653_MOTION_FREEFALL_INT1) ) return false; 
