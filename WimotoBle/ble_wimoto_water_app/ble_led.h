 ...
 *          through the ble_led_battery_level_update() function.
 ...
 *       ble_led_on_ble_evt() from the from the @ref ble_stack_handler callback.
 */
...
typedef enum
{
    ble_led_EVT_NOTIFICATION_ENABLED,                             /**< Battery value notification enabled event. */
    ble_led_EVT_NOTIFICATION_DISABLED                             /**< Battery value notification disabled event. */
} ble_led_evt_type_t;
...
typedef void (*ble_led_evt_handler_t) (ble_led_t * p_bas, ble_led_evt_t * p_evt);

/**@brief Battery Service init structure. This contains all options and data needed for
 *        initialization of the service.*/
typedef struct
{
    ble_led_evt_handler_t         evt_handler;                    /**< Event handler to be called for handling events in the Battery Service. */
 ...
 */
uint32_t ble_led_init(ble_led_t * p_bas, const ble_led_init_t * p_bas_init);
 ...
 *       ble_led_battery_level_update() must be called upon reconnection if the
 ...
 */
void ble_led_on_ble_evt(ble_led_t * p_bas, ble_evt_t * p_ble_evt);
 ...
 */
uint32_t ble_led_battery_level_update(ble_led_t * p_bas, uint8_t battery_level);


uint32_t ble_led_battery_level_update(ble_led_t * p_bas, uint8_t battery_level);
uint32_t ble_battery_level_check(ble_led_t * p_bas);
