...
*/
static void on_connect(ble_led_t * p_bas, ble_evt_t * p_ble_evt)
{
...
* @param[in]   p_ble_evt   Event received from the BLE stack.
*/
static void on_disconnect(ble_led_t * p_bas, ble_evt_t * p_ble_evt)
{
...
* @param[in]   p_ble_evt   Event received from the BLE stack.
*/
static void on_write(ble_led_t * p_bas, ble_evt_t * p_ble_evt)
{
    if (p_bas->is_notification_supported)
    {
                ...
                )
        {
            ...
            if (p_bas->evt_handler != NULL)
            {
                ...
                {
                    evt.evt_type = ble_led_EVT_NOTIFICATION_ENABLED;
                }
                else
                {
                    evt.evt_type = ble_led_EVT_NOTIFICATION_DISABLED;
                }
...
*/
static uint32_t battery_level_char_add(ble_led_t * p_bas, const ble_led_init_t * p_bas_init)
{
    uint32_t            err_code;
