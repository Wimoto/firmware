...
bool     				m_soils_alarm_ind_conf_pending = false;       /**< Flag to keep track of when an indication confirmation is pending.*/
...
static void on_soils_evt(ble_soils_t * p_soils, ble_soils_alarm_evt_t *p_evt)
{
    switch (p_evt->evt_type)
    {
        case BLE_SOILS_EVT_INDICATION_ENABLED:
            // Indication has been enabled, send a single temperature measurement
            //temperature_measurement_send();
            break;

        case BLE_SOILS_EVT_INDICATION_CONFIRMED:
            m_soils_alarm_ind_conf_pending = false;
            break;
...
static void on_write(ble_soils_t * p_soils, ble_evt_t * p_ble_evt)
{
    ...
    if (p_soils->is_notification_supported)
    {
                ...
                )
        {
            ...
            if (p_soils->evt_handler != NULL)
            {
                ble_soils_alarm_evt_t evt;

                if (ble_srv_is_indication_enabled(p_evt_write->data))
                {
                    evt.evt_type = BLE_SOILS_EVT_INDICATION_ENABLED;
                }
                else
                {
                    evt.evt_type = BLE_SOILS_EVT_INDICATION_DISABLED;
                }

                on_soils_evt(p_soils, &evt);
            }
        } 
 ...
 *
 * @param[in]   p_soil         soil moisture Service structure.
 * @param[in]   p_ble_evt      Event received from the BLE stack.
 */
static void on_hvc(ble_soils_t * p_soils, ble_evt_t * p_ble_evt)
{
    ble_gatts_evt_hvc_t * p_hvc = &p_ble_evt->evt.gatts_evt.params.hvc;
    if (p_hvc->handle == p_soils->soil_mois_alarm_handles.value_handle)
    {
        ble_soils_alarm_evt_t evt;
        evt.evt_type = BLE_SOILS_EVT_INDICATION_CONFIRMED;
        on_soils_evt(p_soils, &evt);
    }
}
...
static uint32_t soil_mois_alarm_char_add(ble_soils_t * p_soils, const ble_soils_init_t * p_soils_init)
{
    ...
    char_md.char_props.read   = 1;
    char_md.char_props.indicate = (p_soils->is_notification_supported) ? 1 : 0;
    char_md.p_char_pf         = NULL;
    char_md.p_user_desc_md    = NULL;
    char_md.p_cccd_md         = (p_soils->is_notification_supported) ? &cccd_md : NULL;
    char_md.p_sccd_md         = NULL;
    ...
    {		
          ...
          if ((p_soils->conn_handle != BLE_CONN_HANDLE_INVALID) && p_soils->is_notification_supported)
          {
            ...
            err_code = sd_ble_gatts_hvx(p_soils->conn_handle, &hvx_params);
						...
						if(err_code == NRF_SUCCESS)
						{
							m_soils_alarm_ind_conf_pending = true;
						}
